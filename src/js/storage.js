...
function addMovieToStorage(key, movie, button) {
  console.log("addMovieToStorage :>>", movie);
...
function onAddWatchedBtn(event) {
  console.log("Обробка натискання ADD TO WATCHED :>>", event);
...
function onAddQueueBtn(event) {
  console.log("Обробка натискання ADD TO QUEUE :>>", event);
...
export function storageHasMovie(storageKey, movieID) {
  console.log("Обробка storageHasMovie :>>", movieID);
  let storageMoviesStr;
  try {
